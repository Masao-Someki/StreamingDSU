...
class TTSExp2(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # Frozen parameter
        self.clustering_head = nn.Linear(768, 500)
        self.loss = nn.CrossEntropyLoss(ignore_index=-1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        units = self.clustering_head(feats) # (B, T, Cluster)
        print(sids.shape, units.shape)
        ce_loss = self.loss(units.transpose(1, 2), sids)
        
        acc = 0
        for b in range(units.shape[0]):
            sids_length = torch.sum(sids[b] != -1)
            selected_cls = torch.argmax(units[b], dim=-1)
            acc += torch.sum(selected_cls == sids[b]) / sids_length
